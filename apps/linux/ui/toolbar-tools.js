 ...
 * that something is happening *now*, so a growing number is the information. Rounded
 * rather than ticking to the millisecond: a counter that changes ten times a second
 * reads as an error condition.
 */
function heldSaid(forMs) {
  const secs = Math.max(0, Math.round(forMs / 1000));
...
/**
 * Which window an agent is holding, named for somebody who is looking at the screen.
 *
 * A bare X id is true and useless. The one window the toolbar can always name is the
 * one in front, and that is also the one that matters most — "an agent is working in
 * the window you are looking at" is the sentence somebody needs before they reach for
 * the keyboard. Everything else stays the id, because inventing a name for a window
 * nobody asked about would be a guess presented as a fact.
 */
function heldWhere(surface, front) {
  if (front && front.id && surface === front.id) {
    return front.title ? `${front.title} — in front of you` : "the window in front of you";
  }
  return surface;
}
